...
function __construct ($id, $name, $surname, $birthDay, $sex, $city) {
	...
	if (gettype($name) != "string" || !preg_match('/^[a-zA-Zа-яА-Я ]+$/', $name) || $name == "")
	$msg = $msg . " Имя должно состоять из букв и не может быть пустым.";
	else
	$this->name = $name;

	if (gettype($surname) != "string" || !preg_match('/^[a-zA-Zа-яА-Я ]+$/', $surname) || $surname == "")
	$msg = $msg . " Фамилия должна состоять из букв и не может быть пустой строкой.";
	else
	$this->surname=$surname;

	if (gettype($birthDay) != "string" || !preg_match('/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/', $birthDay) || $birthDay == "")
	...
	if (gettype($city) != "string" || !preg_match('/^[a-zA-Zа-яА-Я ]+$/', $city) || $city == "")
	$msg = $msg . " Имя должно состоять из букв и не может быть пустым.";
	else
	$this->city=$city;
...
function saveUser() {
	...
	$query = $mysqli->query("INSERT INTO `users` VALUES('$this->id', '$this->name', '$this->surname', '$this->birthDay', '$this->sex', '$this->city')");
...
$mysqli = new Mysqli($server, $user, $pas, $db); 
$mysqli->query("SET CHARSET utf8");
$query = $mysqli->query("SELECT 'id', 'name', 'surname', 'birthDay', 'sex', 'city' FROM `users` WHERE 'id '= $id");
...
if (mysqli_num_rows($query) != 0) {
	...
	else echo "Пользователя с таким ID нет в базе";
...
function updateUser($id, $name, $surname, $birthDay, $sex, $city) {

	$mysqli = new Mysqli($server, $user, $pas, $db); 
	$mysqli->query("SET CHARSET utf8");
	$query = $mysqli->query('UPDATE users SET name = "' . $this->name . '" , surname = "' . $this->surname . '" , birthDay = "' . $this->birthDay . '" , sex = "' . $this->sex . '" , city = "' . $this->city . '" WHERE id = "' . $this->id . '"');

}

static function age($User) {
...
// print_r(user::realSex($pers));
// print_r(user::age($pers));
